// Favorites.jsx - Shows only favorited notes (notes remain visible in Home)
...
export default function Favorites({ navigation }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.warn("Favorites:init error", err);
      }
    })();
  ...
  const fetchNotes = async (userId) => {
    ...
    try {
      let query = supabase
        .from("notes")
        .select("*")
        .eq("favorite", true)  // ✅ ONLY fetch favorited notes
      ...
      console.error("Error fetching notes (Favorites):", err);
  ...
  const favoriteNotes = useMemo(
    () => notesInSpace.filter((n) => n.favorite === true || n.favorite === "true"),
    [notesInSpace]
  );
  ...
  const handleRemoveFavorite = async (note) => {
    try {
      const updateData = { 
        favorite: false,
      ...
      CustomAlert.alert("Success", "Removed from favorites.", [{ text: "OK" }]);
      fetchNotes(user?.id);
    } catch (err) {
      console.error("Error removing favorite:", err);
      CustomAlert.alert("Error", err.message || "Failed to remove favorite.", [{ text: "OK" }]);
    }
  };
  ...
  const handleArchiveNote = async (note) => {
    try {
      ...
      CustomAlert.alert("Success", "Note archived successfully.", [{ text: "OK" }]);
      fetchNotes(user?.id);
    } catch (err) {
      console.error("Error archiving note:", err);
      CustomAlert.alert("Error", err.message || "Failed to archive note.", [{ text: "OK" }]);
    }
  };
  ...
  const handleRemoveSelectedFromFavorites = useCallback(() => {
    ...
    Alert.alert(
      ...
      `Remove ${selectedNoteIds.length} selected note${selectedNoteIds.length > 1 ? "s" : ""} from favorites?`,
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              CustomAlert.alert("Success", "Selected notes removed from favorites.", [
                { text: "OK" },
              ]);
              ...
              CustomAlert.alert(
                "Error",
                err.message || "Failed to remove selected notes from favorites.",
                [{ text: "OK" }]
              );
              fetchNotes(user?.id);
            } finally {
    ...
    return (
      <View style={styles.emptyContainer}>
        ...
        <Text style={styles.emptySubtitle}>
          {selectedSpaceId
            ? "No favorite notes in this space yet."
            : "You haven't favorited any notes yet. Swipe right on a note to add it to favorites."}
        </Text>
      </View>
    );
  };
          ...
          <View style={styles.multiSelectActions}>
            ...
            <TouchableOpacity
              style={[
                ...
                styles.multiSelectRemoveButton,
                (bulkRemoving || selectedNoteIds.length === 0) &&
                  styles.multiSelectButtonDisabled,
              ]}
              onPress={handleRemoveSelectedFromFavorites}
              activeOpacity={0.8}
              disabled={bulkRemoving || selectedNoteIds.length === 0}
            >
              <Text style={styles.multiSelectRemoveText}>
                {bulkRemoving ? "Removing..." : `Remove (${selectedNoteIds.length})`}
              </Text>
            </TouchableOpacity>
          </View>
        </View>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <NoteCard
            ...
            isArchived={false}  // ✅ Favorites are active notes
          />
        )}
        ...
        contentContainerStyle={
          favoriteNotes.length === 0 ? styles.flatListEmpty : styles.flatListContent
        }
        showsVerticalScrollIndicator={false}
      />
    </Animated.View>
...
const styles = StyleSheet.create({
  ...
  multiSelectRemoveButton: {
    borderColor: "rgba(250,204,21,0.6)",
    backgroundColor: "rgba(250,204,21,0.15)",
    marginLeft: 12,
  },
  multiSelectRemoveText: {
    color: "#facc15",
    fontSize: 14,
    fontWeight: "700",
  },
