...
export default function Reminders({ navigation }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.warn("Reminders:init error", err);
      }
    })();
  ...
  const fetchSpaces = async (userId) => {
    try {
      let query = supabase.from("spaces").select("id,name,icon,color").order("created_at", { ascending: false });
  ...
  const fetchNotes = async (userId) => {
    ...
    try {
      ...
      console.error("Error fetching notes (Reminders):", err);
  ...
  // Helper to detect reminder presence across various field names
  const noteHasReminder = (n) => {
    if (!n) return false;
    return Boolean(
      n.reminder_at ||
      n.reminderAt ||
  ...
  const reminderNotes = useMemo(
    () => notesInSpace.filter(noteHasReminder),
    [notesInSpace]
  );

  // Optional: try to sort by reminder timestamp if available (best-effort)
  const sortedReminderNotes = useMemo(() => {
    const parseReminderTime = (n) => {
      const candidates = [n.reminder_at, n.reminderAt, n.reminder_time, n.reminderTime, n.reminderDate, n.reminder];
    ...
    return [...reminderNotes].sort((a, b) => {
      const ta = parseReminderTime(a);
      const tb = parseReminderTime(b);
      // put those with valid reminder timestamps first (earlier reminders first)
    ...
    return (
      <View style={styles.emptyContainer}>
        ...
        <Text style={styles.emptySubtitle}>
          {selectedSpaceId ? "No reminders in this space yet." : "You have no reminders set."}
        </Text>
      </View>
    );
  };
    ...
    <Animated.View style={[styles.container, { opacity: fadeAnim }]}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerContent}>
          ...
          <Text style={styles.headerSubtitle}>
            {reminderNotes.length} reminder{reminderNotes.length === 1 ? "" : "s"}
          </Text>
        </View>
      ...
      >
        <View style={styles.spaceModalOverlay}>
          <TouchableOpacity style={styles.spaceModalBackdrop} activeOpacity={1} onPress={() => setShowSpaceModal(false)} />
          <View style={styles.spaceModalCard}>
            <View style={styles.spaceModalHeader}>
              <Text style={styles.spaceModalTitle}>Select Space</Text>
              <TouchableOpacity style={styles.spaceModalClose} onPress={() => setShowSpaceModal(false)}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.spaceOptionsContainer}>
              <TouchableOpacity
                ...
                onPress={() => { setSelectedSpaceId(null); setShowSpaceModal(false); }}
                activeOpacity={0.8}
              >
              ...
              <TouchableOpacity
                ...
                onPress={() => { setSelectedSpaceId("no-space"); setShowSpaceModal(false); }}
                activeOpacity={0.8}
              >
              ...
              {spaces.length === 0 ? (
                <View style={styles.spaceEmptyState}>
                  ...
                  <TouchableOpacity style={styles.spaceEmptyButton} onPress={() => { setShowSpaceModal(false); navigation.navigate("Spaces"); }} activeOpacity={0.8}>
                ...
                spaces.map((space, idx) => {
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      onPress={() => { setSelectedSpaceId(space.id); setShowSpaceModal(false); }}
                      activeOpacity={0.8}
                    >
              ...
              {spaces.length > 0 && (
                <TouchableOpacity style={styles.manageSpacesButton} onPress={() => { setShowSpaceModal(false); navigation.navigate("Spaces"); }} activeOpacity={0.8}>
      ...
      <FlatList
        ...
        contentContainerStyle={sortedReminderNotes.length === 0 ? styles.flatListEmpty : styles.flatListContent}
